...
const ProfileForm: React.FC<ProfileFormProps> = ({
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      const { name, username, imageUrl } = await dispatch(
    ...
    <div className=" overflow-auto w-full md:w-[65%] tab:w-[65%] lg:w-[62%] p-1">
      <div className="flex justify-end mt-2 md:hidden ">
        <button
          ...
          className="py-2 px-3 border-2 rounded-md border-slate-300 text-center hover:bg-white hover:border-white"
        >
        ...
        {openEdit && (
          <div className="flex gap-1 mt-2 text-center">
            <div
              ...
              className="cursor-pointer btn text-md w-1/2 bg-yellow-500 p-[15px] rounded-md"
            >
              Cancel
            </div>
            <button
