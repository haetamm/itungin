...
export default function NavBar() {
  ...
  return (
    <>
      <div className="text-sm font-semibold wrap-navbar flex justify-end xs:justify-between">
        <div className="flex space-x-3 items-center">
          {navItems.map((item, index) => (
            <div
              key={index}
              className="hidden xs:flex item-center space-x-1 items-center px-2 py-1.5 btn-navbar cursor-pointer"
            >
              {item.icon}
              <div>{item.label}</div>
            </div>
        ...
        <div className="flex items-center space-x-3">
          <div>
            <div
              ...
              className={`${openDropdown ? 'text-blue-400' : ''} flex items-center space-x-2 cursor-pointer`}
            >
              <div className="items-center text-end">
                <p className="text-lg hidden lg:inline-block">Tarak Company</p>
              ...
              {openDropdown && (
                <ul className="dropdown-list text-md">
                  <Link
                    to={'/profile'}
                    onClick={handleDropdownToggle}
                    className={`${isActive ? 'active-link' : ''} dropdown-list-item flex items-center space-x-2`}
                  >
                    <FaRegUserCircle />
                    <div>Profile</div>
                  </Link>
