...
export class PayablePaymentService {
  ...
  async getPayablePaymentDetail(paymentId: string) {
    const payment = await paymentRepository.getPaymentDetail(paymentId);
    if (!payment) throw new ResponseError(404, 'Payment not found');
    return payment;
  }
  ...
  async createPayablePayment({
    ...
    return await prismaClient.$transaction(async (prismaTransaction) => {
      ...
      // Find the cash journal entry (credit) for the Payment record
  ...
  async updatePayablePayment(
    ...
    return await prismaClient.$transaction(async (prismaTransaction) => {
      ...
      // === Update entries lama ===
      // Entry 1 → akun utang (debit)
      // Entry 2 → akun kas (kredit)
      ...
      // === Update saldo akun (selisih nominal) ===
  ...
  async deletePayablePayment(paymentId: string): Promise<void> {
    return await prismaClient.$transaction(async (prismaTransaction) => {
      ...
      if (!payable) {
        throw new ResponseError(404, 'Payable not found');
      }
      ...
      // Delete payment record FIRST to satisfy foreign key constraint
      await paymentRepository.deletePayment(paymentId, prismaTransaction);
